    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-2xl">
      ...
      <form>
        <div class="mb-6">
          ...
          <!-- Package -->
          <SelectInput label="Package" :items="PackageTypeSelectItems" v-model="insurancePackage" />
          ...
          <div class="mb-10">
            ...
            <!-- Cancellation -->
            <CheckboxInput label="Cancellation" id="cancellation" v-model="cancellation" />
            <!-- Sport Activities -->
            <CheckboxInput
        ...
        >
          <template #text>
            <h3 class="text-lg font-semibold text-green-800">
              Total Price: {{ nicePrice(totalPrice) }}
            </h3>
          </template>
        </InfoMessages>
        <InfoMessages
...
<script setup lang="ts">
  ...
  const getDateDifference = (): number | undefined => {
    if (periodEnd.value && periodStart.value) {
      return (
        (new Date(periodEnd.value).getTime() - new Date(periodStart.value).getTime()) /
          (1000 * 60 * 60 * 24) +
        1
      )
    }
  ...
  const totalPrice = computed((): number => {
    ...
    if (cancellation.value && insuranceType.value) {
      totalPrice += Number(
        (
          packageForAllPersons * Number(additionalCharge.cancellation?.[insuranceType.value])
        ).toFixed(2)
      )
    }
    ...
    if (sportActivities.value && insuranceType.value) {
      totalPrice += Number(
        (
          packageForAllPersons * Number(additionalCharge.sportActivities?.[insuranceType.value])
        ).toFixed(2)
      )
    }
